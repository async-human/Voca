...
export function VoiceInsightsPanel({ voiceProfile, loading, variant = 'card' }: VoiceInsightsPanelProps) {
  ...
  if (loading) {
    return (
      <div className={cn('rounded-[22px] border border-faint-2 bg-white/60 px-5 py-4 animate-pulse', !isPage && 'mb-6')}>
  ...
  const body = (
    <div className={cn(!isPage && 'border-t border-faint-2/80', 'px-5 pb-5 pt-4 md:px-6 md:pb-6')}>
      {!hasData ? (
        <p className="text-[13px] leading-relaxed text-muted">
          Record a few sessions in Studio and Vokal will learn your style — trait scores, patterns, and clarity trends appear here.
        </p>
      ) : (
  ...
  if (isPage) {
    return (
      <motion.div
        ...
        className="overflow-hidden rounded-[22px] border border-faint-2 bg-white/70 shadow-[0_12px_40px_rgba(28,24,20,.04)] backdrop-blur-sm"
      >
        <div className="flex items-center justify-between border-b border-faint-2/80 px-5 py-4 md:px-6">
          <p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted">
            {hasData ? `${sessions} session${sessions === 1 ? '' : 's'} learned` : 'Getting started'}
          </p>
